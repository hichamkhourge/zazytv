#!/bin/bash
#
# IPTVtune Scheduler - Manages timing logic for auto-runs
#
# This script decides when to run IPTVtune automation based on:
# - 20-hour interval after successful run
# - 1-hour retry interval after failed run
#
# State files (stored in /tmp for simplicity):
# - /tmp/iptvtune_last_success - Unix timestamp of last successful run
# - /tmp/iptvtune_last_attempt - Unix timestamp of last attempt (success or failure)
#

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LAST_SUCCESS_FILE="/tmp/iptvtune_last_success"
LAST_ATTEMPT_FILE="/tmp/iptvtune_last_attempt"
LOG_FILE="/tmp/iptvtune_scheduler.log"
...
run_automation() {
    ...
    log "=========================================="
    log "Starting IPTVtune automation..."
    log "=========================================="
    ...
    # Headless mode, auto-exit. Note: this can run for up to ~3 hours while polling
    # for the credentials email.
    HEADLESS=True AUTO_EXIT=True \
        venv/bin/python iptvtune_automation.py 2>&1 | tee -a "$LOG_FILE"

    local exit_code=${PIPESTATUS[0]}

    if [ "$exit_code" -eq 0 ]; then
        log "[SUCCESS] IPTVtune automation completed successfully"
        log "Updating last success timestamp: $now"
        save_timestamp "$LAST_SUCCESS_FILE" "$now"
        log "Next run scheduled in 20 hours"
    else
        log "[FAILURE] IPTVtune automation failed with exit code $exit_code"
...
# Main execution
main() {
    log "IPTVtune Scheduler started"
    ...
    log "IPTVtune Scheduler finished"
}
